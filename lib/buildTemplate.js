...
const buildFromTemplate = (parentDir, workingDir, selectedTemplate, projectName) => {
  ...
  if (!fs.existsSync(path.join(workingDir, 'README.md'))) fs.writeFileSync(path.join(workingDir, 'README.md'), readmeData);
  if (!fs.existsSync(path.join(workingDir, 'LICENSE'))) fs.writeFileSync(path.join(workingDir, 'LICENSE'), license);
  if (!fs.existsSync(path.join(workingDir, '.gitignore'))) fs.writeFileSync(path.join(workingDir, '.gitignore'), gitignore);
